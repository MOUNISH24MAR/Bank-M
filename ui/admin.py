...
class AdminDashboard(ttk.Frame):
    ...
    def view_user_accounts(self):
        ...
        # Create the button as an instance variable to allow dynamic updates
        self.account_action_btn = ttk.Button(
            btn_frame,
            text="Select an account",
            command=lambda: self.toggle_block_account(tree, user_id, win),
            bootstyle=SECONDARY,  # Default gray when no account is selected
            width=20
        )
        self.account_action_btn.pack(side=LEFT, padx=5)
        
        # Bind selection event to update the button's text and style
        def on_account_select(event):
            selected = tree.focus()
            if not selected:
                self.account_action_btn.configure(text="Select an account", bootstyle=SECONDARY)
                return
            account_data = tree.item(selected)['values']
            current_status = account_data[4]  # "Yes" or "No"
            if current_status == "No":
                self.account_action_btn.configure(text="Block", bootstyle=DANGER)  # Red for block
            else:
                self.account_action_btn.configure(text="Unblock", bootstyle=WARNING)  # Yellow for unblock
        
        tree.bind('<<TreeviewSelect>>', on_account_select)
    ...
    def toggle_block_account(self, tree, user_id, window):
        ...
        if success:
            ...
            # After refresh, no item is selected, so reset the button
            self.account_action_btn.configure(text="Select an account", bootstyle=SECONDARY)
        else:
